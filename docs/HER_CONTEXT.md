...
- Reworked the China/USA category descriptions into mirrored row-based logistics tables. Every physical equipment component has its own queue, in-transit, rolling-seven-day, and cumulative fields; tungsten now exposes the same storage/transit/seven-day/cumulative stages. Route telemetry separately reports each route's capacity and the shared import/export pool's daily limit, used capacity, remaining capacity, and utilization percentage.
- Tungsten repayment no longer waits for free space in the US warehouse before leaving China. A `1000`-unit packet consumes Chinese export capacity and enters a seven-day transit variable immediately; only the arrival event adds it to the US warehouse. This prevents destination-storage updates from producing misleading multi-thousand-unit dispatch bursts and keeps route utilization tied to actual departures from China.
...
## 2026-09-06 — China aid logistics balance pass

- Western-route throughput is now spent once per week. Every route maintains separate import and export allocations and reports its own used/available capacity in both the Chinese and American decision-category tables.
- Cargo consumes capacity according to HER equipment transport weight (`lend_lease_cost × 5`): small arms and ammunition `0.5`, support equipment `1.25`, trucks `5`, field/AT/AA guns `10`, heavy artillery `15`, and aircraft `40` per item. One thousand tungsten consumes `250` capacity.
- Dispatch packets cost: infantry `445`, artillery `1500`, ammunition `500`, support equipment `250`, trucks `1000`, aircraft `200`, and tungsten `250` capacity. Up to twelve packets of each eligible class can leave during one weekly logistics tick.
- American equipment spends 14 days travelling to the BRA transit depot. After dispatch from BRA it spends another 21 days by sea, 28 through Indochina, 35 over Burma or Ledo, or 14 over the Hump. Tungsten reaches the United States after 35 days by sea, 42 through Indochina, or 49 over Burma/Ledo.
- Contract prices are now: infantry `55000`, artillery `70000`, ammunition `45000`, support equipment `45000`, trucks `35000`, fighters `50000`, CAS `55000`, and light bombers `65000` American credit.
- Existing focus integration: the Burma route opens after either China's `CHI_burma_road` or Britain's `uk_burma_focus`, provided the physical route states remain outside enemy control. China's `CHI_ledo_road` approves the later Ledo project. The British Burma focus also exposes the aid interface and enables subsequent US/UK Burma-route expansion decisions, so old saves and either national path work without duplicate route flags.
- Ledo chronology correction: `CHI_ledo_road` now approves the project but no longer opens the route. The focus is locked until 1942-11-30; after its 70-day preparation, USA or ENG may begin a 720-day construction decision. The complete focus-plus-decision project therefore lasts about 790 days, and the route is usable no earlier than 1945-01-28 while Assam, Mandalay, and Yunnan remain outside enemy control. Initial weekly capacity is `1000`, with three later `+500` expansions to a maximum of `2500`.
- Flying Tigers integration reuses the existing `CHI_invite_the_flying_tigers` → `wtt_china_shared.10` American-consent chain. Once USA accepts, its aid category offers a 60-day AVG deployment decision requiring 120 fighters. Three 40-aircraft packages then enter the ordinary BRA staging and route pipeline; China receives `CHI_flying_tigers_air_group` (`+5%` air mission efficiency, `+25%` ace generation, `-20%` air-wing XP loss on casualties).
- Credit progression now has three stages: the original `500000` credit/`1000000` tungsten obligation; a Chinese-requested 1941 expansion of `750000`/`1500000` after at least `100000` tungsten has arrived and the balance falls to `250000`; and a later USA-offered emergency credit of `1000000`/`1000000`, accepted or refused by China after the expanded balance falls to `300000`.
- `CHI_invite_the_flying_tigers` was moved from the inaccessible shared-China focus pool into `china_nationalist_focus` and attached directly to `CHI_war_of_resistance`, preserving the existing American-consent event chain.
- Ledo construction, Ledo expansion, and Burma Road expansion are single global projects. Once USA or ENG starts one, a Chinese global in-progress flag prevents the other sponsor from launching a duplicate; China and the non-sponsoring Allied power receive a synchronized, non-interactive timed mission showing the remaining work.
- Burma-route activation is historically gated: even after China completes `CHI_burma_road` or Britain completes `uk_burma_focus`, the physical route and its USA/ENG expansion decision do not become available before 2 December 1938. The nationalist Chinese `CHI_invite_the_flying_tigers` focus is unavailable before 15 April 1941; its focus duration, American deployment decision, and physical transit delay push aircraft arrival toward late 1941.
- `CHI_the_hump` and `CHI_ledo_road` are both native focuses in `china_nationalist_focus` (no duplicate IDs remain in `china_shared`). Their historical dates are enforced on the focuses themselves: the Hump after 7 April 1942 and Ledo after 30 November 1942. USA/ENG construction and expansion decisions are unlocked through the completed Chinese focus. Hump expansion now uses the same single-sponsor/global-observer pattern as both road projects.
