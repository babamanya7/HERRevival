...
- Existing focus integration: the Burma route opens after either China's `CHI_burma_road` or Britain's `uk_burma_focus`, provided the physical route states remain outside enemy control. China's `CHI_ledo_road` opens the Ledo route under the same map-control principle. The British Burma focus also exposes the aid interface and enables subsequent US/UK Burma-route expansion decisions, so old saves and either national path work without duplicate route flags.
