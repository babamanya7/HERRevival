...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT_BORDER Input ) : PDX_COLOR
		{
			...
			float vPulseFactor = saturate( smoothstep( 0.0f, 1.2f, ( 1.0f - abs( Input.uv.x - 0.5f ) ) + sin( vTime_Transparency.x * 0.0f ) * 0.15f ) ) * vSelectionColor.a; 
			...
			return float4( vColor.rgb, max( vColor.a, vPulseFactor - 1.0f ) * vTime_Transparency.y );
		}
